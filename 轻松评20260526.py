...
class PriceMatchingSystem(tk.Toplevel):
    ...
    def parse_filename_info(self, file_path):
        ...
        # 从文件名直接提取市场类型（作为备选）
        filename_upper = stem.upper()
        if '沪市' in stem or '600' in stem[:3] or '601' in stem[:3] or '603' in stem[:3] or '688' in stem[:3]:
            result['market'] = '沪市'
        elif '深市' in stem or '000' in stem[:3] or '001' in stem[:3] or '002' in stem[:3] or '300' in stem[:3]:
            result['market'] = '深市'
        elif '北京市场' in stem or '北交所' in stem or '835' in stem[:3] or '920' in stem[:3]:
            result['market'] = '北京市场'
    ...
    def match_market_price(self, transaction_file, output_root, input_root=None):
        """
        返回结果列表，每个元素对应一个 (证券代码, 市场类型, 数据来源) 的匹配明细。
        输出文件保持原文件夹结构，汇总表中每行指向同一输出文件。
        包含沪市股票（无需匹配）的记录。
        """
        try:
            ...
            for sheet_name in xl.sheet_names:
                ...
                if market_type is None:
                    market_type = file_market_type  # 使用文件名解析的市场类型
            ...
            # ==================== 修复核心：改为复合键 (stock_code, market_type, data_source) 分组 ====================
            ...
            for (sheet_name, idx, stock_code, short_name, company_name, data_source, market_type) in all_stock_info:
                key = (stock_code, market_type, data_source)
                ...
                if key not in group_info:
                    ...
                    if company_name != "未知" and group_info[key]["company_name"] == "未知":
                        group_info[key]["company_name"] = company_name
                if short_name:
                    group_info[key]["short_names"].add(short_name)

            # 价格缓存简称映射（保持按 stock_code 维度，映射关系与市场类型无关）
            code_to_names = {}
            for _, _, stock_code, short_name, _, _, _ in all_missing_records:
            ...
            for sheet_name, idx, stock_code, short_name, date_str, mkt, src in all_missing_records:
            ...
            if group_info:
                ...
                for key in group_info.keys():
                    stock_code, market_type, data_source = key
                    if market_type == "沪市":
            ...
            if input_root:
                ...
                # 单文件模式：放入"市场日均价匹配结果YYYYMMDD"子文件夹
            ...
            with pd.ExcelWriter(out_file, engine="openpyxl") as writer:
                for sheet_name, df in filled_sheets.items():
                    # 数值格式转换：确保关键列为数值类型，避免Excel绿色三角提示
            ...
            for key, info in group_info.items():
                stock_code, market_type, data_source = key
                company_name = info["company_name"] if info["company_name"] != "未知" else "未知"
                short_names = "/".join(sorted(info["short_names"])) if info["short_names"] else "未知"
                total_stock_trans = len(group_records[key])
            ...
            for idx, input_file in enumerate(files_to_process):
                ...
                if rows:
                    ...
                    for row in rows:
                        status = row["匹配状态"]
                        if status == "无需匹配":
                            self.log(f"  ℹ {row['证券代码']}({row['证券简称']}) [{row['市场类型']}-{row['数据来源']}] - {status}", "info")
                        elif status == "全部匹配":
                            self.log(
                                f"  ✓ {row['证券代码']}({row['证券简称']}) [{row['市场类型']}-{row['数据来源']}] - {status} ({row['已匹配交易条数']}/{row['需匹配交易条数']})",
                                "success")
                        elif status == "部分匹配":
                            self.log(
                                f"  ⚠ {row['证券代码']}({row['证券简称']}) [{row['市场类型']}-{row['数据来源']}] - {status} ({row['已匹配交易条数']}/{row['需匹配交易条数']})",
                            ...
                            self.log(f"  ✗ {row['证券代码']}({row['证券简称']}) [{row['市场类型']}-{row['数据来源']}] - {status} (0/{row['需匹配交易条数']})",
                                     "error")
                else:
...
class TaxCalculator(tk.Toplevel):
    def __init__(self, master):
        ...
        self.batch_files = []
        self.batch_results = {}
        self.batch_output_dir = tk.StringVar()  # 新增：批量导出路径变量
        ...
        ttk.Label(self.input_frame, text="根据以下政策确定的增值税买入价:").grid(row=0, column=0, padx=5, pady=5,
                                                                                 sticky="e")
...
四、单位将其持有的限售股在解禁流通后对外转让，按照《国家税务总局关于营改增试点若干征管问题的公告》(2016年第53号)第五条规定确定的买入价，低于该单位取得限售股的实际成本价的，以实际成本价为买入价计算缴纳增值税。"""
        ...
        ttk.Entry(path_frame, textvariable=self.batch_output_dir, width=50).pack(side=tk.LEFT, padx=5, fill=tk.X,
                                                                                 expand=True)
    ...
    def calculate_vat_summary(self, df, bureau, company, code='未知', name='未知'):
        """增值税汇总：按月汇总，并计算应申报额（跨月结转负数，跨年清零），按年度、月份排序"""
        sales_df = df[df['单笔卖出金额'] > 0].copy()
        if sales_df.empty:
    ...
    def calculate_income_summary(self, df, bureau, company, code='未知', name='未知'):
        """所得税汇总：按年汇总，不显示月份列，同一年度只显示一条数据，结果为同一年度所有月份的数据之和"""
        sales_df = df[df['单笔卖出金额'] > 0].copy()
        if sales_df.empty:
    ...
    # 修改：流通股批量计算并直接导出（先选路径，再计算导出，显示进度条，不弹窗）
        ...
        def calculate_and_export_in_thread():
            ...
            for i, file_path in enumerate(self.batch_files):
                ...
                try:
                    ...
                    # 收集汇总数据
                    sales_df = result_df[result_df['单笔卖出金额'] > 0].copy()
